...
Random si comportino in maniera simile. Ciò è strano poiché ci si aspetterebbe che 
l'agente DQN, da un certo punto in poi, impari a prendere decisioni migliori rispetto
a un agente Random.

Se si analizza \autoref{fig:pktDrop_allAgents_defaultConfig} si può notare che l'agente
DQN perde meno pacchetti rispetto al random in uno stesso timestep, quindi è possibile
che i risultati ottenuti precedentemente confrontando le reward cumulative sia dovuto
al fatto che l'agente DQN invia più pacchetti rispetto al random, quindi consuma più
energia e conseguentemente è penalizzato di più.

La \autoref{fig:cumulativeReward_noEnergyPenalty_allAgents_120s} sembra confermare la
suddetta tesi: azzerando il peso del termine energetico nella reward, l'agente DQN
sembra andare meglio del random in termini di reward cumulativa. É possibile che il
modello energetico utilizzato di default nella simulazione preveda una batteria
inadeguata rispetto ai consumi imposti dalla NIC utilizzata per l'invio dei pacchetti,
e quindi il ricorso al Power Chord comporti spesso delle penalità.
...
\begin{figure*}
    \centering
    \includesvg[width=\textwidth]{figs/results_charts/cumulative_reward_dqnDeep_dqnFlat_random_defaultConfig}
...
\begin{figure*}
    \centering
    \includesvg[width=\textwidth]{figs/results_charts/pktDrop_allAgents_defaultConfig.svg}
    \caption{Confronto packet drop tra agenti DQN con decision, DQN semplice ("flat") e Random.}
    \label{fig:pktDrop_allAgents_defaultConfig}
\end{figure*}
\begin{figure*}
    \centering
    \includesvg[width=\textwidth]{figs/results_charts/cumulativeReward_noEnergyPenalty_allAgents_120s.svg}
    \caption{Confronto cumulative reward tra agenti DQN con decision, DQN semplice ("flat") e Random, con peso termine energetico reward a 0, i restanti a 1/2.}
    \label{fig:cumulativeReward_noEnergyPenalty_allAgents_120s}
\end{figure*}
