...
\begin{itemize}
    ...
    \item Sample batch size: la dimensione del minibatch da prelevare dal replay buffer. Nel nostro caso è stato scelto 100 come la train frequency in modo da prendere in media lo stesso numero di esperienze inserite nel buffer ed evitare così di perdere troppe esperienze.
\end{itemize}
Tuttavia in seguito alla lettura del paper "The importance of experience replay database composition in deep reinforcement learning"\cite{b1} abbiamo deciso di fare dei cambiamenti all'implementazione del buffer. Il paper in questione mostra che per avere una buon stabilità nel processo di learning è necessario avere un replay buffer composto da esperienze abbastanza diversificate. Nella nostra situazione accadeva spesso che l'agente non imparasse abbastanza in fretta quello che succedeva e che quindi lo stato tendesse a convergere ad una situazione molto specifica (e.g esempio le code piene). Questo faceva si che replay buffer si andasse a riempire di esperienze molto simili, bloccando l'apprendimento dell'agente; l'esplorazione non è risultata sufficiente ad uscire da questa situazione in quanto essa era componeva solo una minima parte del buffer. Per risolvere questo problema ed usare i criteri di diversificazione proposti dal paper, abbiamo deciso di dividere il replay buffer in due parti, una contenente solo esperienze di esplorazione ed una contenente solo esperienze di exploitation. In questo modo è possibile addestrare l'agente con esperienze diverse senza compromettere la quantità di esplorazione e facendo in modo che esso riesca sempre a vedere esperienze abbastanza variegate anche nel caso il buffer di explotation contenga solo esperienze molto simili tra loro.

\subsubsection{Politica di esplorazione}
Come politica di esplorazione abbiamo usato una politica $\epsilon$-greedy. Questa politica permette all'agente di esplorare lo spazio delle azioni con una probabilità $\epsilon$ e di sfruttare la conoscenza acquisita con una probabilità $1 - \epsilon$. %La probabilità $\epsilon$ è stata impostata fissa a 0.1 perché la libreria non permetteva nativamente di impostare un decay rate per $\epsilon$.
...
\subsubsection{Gamma}
Il parametro gamma è stato impostato a 0.99. Questo parametro è importante perché definisce il peso che viene dato alle reward future rispetto alle reward attuali. Un valore di gamma vicino a 1 indica che l'agente darà molto peso alle reward future, mentre un valore di gamma vicino a 0 indica che l'agente darà molto peso alle reward attuali. In generale, un valore di gamma vicino a 1 è consigliato per problemi in cui le reward future sono importanti, mentre un valore di gamma vicino a 0 è consigliato per problemi in cui le reward future sono meno importanti.
...
\begin{thebibliography}{00}
    \bibitem{b1} Bruin, T. D., Jens Kober, and K. Tuyls. \href{https://rll.berkeley.edu/deeprlworkshop/papers/database_composition.pdf}{"The importance of experience replay database composition in deep reinforcement learning."} Computer Science, 2015, pp. 5-8.
\end{thebibliography}
...
\begin{figure*}
    \centering
    \includesvg[width=\textwidth]{figs/results_charts/cumulativeReward_default_allAgents.svg}
...
\begin{figure*}
    \centering
    \includesvg[width=\textwidth]{figs/results_charts/sendCount_allAgents_defaultConfig.svg}
...
\begin{figure*}
    \centering
    \includesvg[width=\textwidth]{figs/results_charts/energySaving_defaultConfig_allAgents.svg}
...
\begin{figure*}
    \centering
    \includesvg[width=\textwidth]{figs/results_charts/occupancies_allAgents_defaultConfig.svg}
...
\begin{figure*}
    \centering
    \includesvg[width=\textwidth]{figs/results_charts/noPowerSaving_allAgents_defaultConfig.svg}
...
\end{figure*}
\begin{figure*}
    \includesvg[width=\textwidth]{figs/results_charts/allPowerSaving_allAgents_defaultConfig.svg}
...
\end{figure*}
\begin{figure*}
    \includesvg[width=\textwidth]{figs/results_charts/balanced_allAgents_5queues_300s.svg}
